...
namespace Iwanna {
	...
	void StageManager::loadGameObjects(String fileName) {
		...
		if (fileName == U"tutorial" || fileName == U"tutorialTrap") {
			...
			if (fileName == U"tutorial" && Global::canUseHaibokusyaMode()) {
		...
		if (fileName == U"ExMiluArea") {
			gameObjects.signs << std::make_shared<Sign>(Vec2{ 2,2 }, SignType::Extra_PracticeMuteki);
		}
